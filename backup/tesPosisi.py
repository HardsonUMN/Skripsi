...
previous_encoder_counts = [0, 0, 0, 0]  # Initialize previous_encoder_counts
...
WHEEL_DIAMETER = 0.0601  # Example wheel diameter in meters
ENCODER_COUNTS_PER_REV = 400  # Example encoder counts per revolution
WHEEL_BASE = 0.325  # Distance between the left and right wheels
...
# Function to detect diagonal movement
def detect_diagonal_movement():
    global encoder_counts
    ...
    # Identify diagonal movement patterns
    if distances[0] > 0 and distances[3] > 0 and distances[1] == 0 and distances[2] == 0:
        movement_direction = "Forward-Right Diagonal"
    elif distances[0] < 0 and distances[3] < 0 and distances[1] == 0 and distances[2] == 0:
        movement_direction = "Backward-Left Diagonal"
    elif distances[0] < 0 and distances[3] < 0 and distances[1] > 0 and distances[2] > 0:
        movement_direction = "Forward-Left Diagonal"
    elif distances[0] == 0 and distances[3] == 0 and distances[1] < 0 and distances[2] < 0:
        movement_direction = "Backward-Right Diagonal"    
    else:
        movement_direction = "Unknown"

    sys.stdout.write("Movement direction: {}\n".format(movement_direction))
...
def update_position():
    ...
    sys.stdout.write("Turn direction: {}\n".format(turn_direction))

    # Detect diagonal movement direction
    detect_diagonal_movement()
...
try:
    while True:
        ...
        sys.stdout.write("Position: ({:.2f}, {:.2f}) Theta: {:.2f}\n".format(x_pos, y_pos, theta))
       

        # Check if any encoder has reached its target count and stop the corresponding motor
        for i in range(len(encoder_channels)):
            if encoder_counts[i] >= 4000:  # 3786 counts per rotation * 10 rotations
                drive_motor(i, 0, 0)

        # Short delay to allow for periodic updates
        time.sleep(0.1)

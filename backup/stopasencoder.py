...
def convert_8bit_to_16bit_pwm(pwm_8bit):
    ...
    const_front_bw = 0.909
    const_back_bw = 0.91322 #backkkk"""
    ...
    while True:
        if poll_obj.poll(0):
            ...
            if movement:
                ...
                #encoder_counts = [0, 0, 0, 0]
                 # Read encoder counts continuously
                for i in range(len(encoder_channels)):
                    print("Encoder", i+1, "Count:", encoder_counts[i])

        if encoder_counts[0] == 4000:
            drive_motor(0, 0, 0)#
            print("stop 1")
        if encoder_counts[1] == 4000:
            drive_motor(1, 0, 0)
            print("stop 2")
        if encoder_counts[2] == 4000:
            drive_motor(2, 0, 0)
            print("stop 3")
        if encoder_counts[3] == 4000:
            drive_motor(3, 0, 0)
            print("stop 4")
            
            #encoder_counts = [0, 0, 0, 0]
        """elif encoder_counts[0] <= -1000:
            stop_motors()
            encoder_counts = [0, 0, 0, 0]"""

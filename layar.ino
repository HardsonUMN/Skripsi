...
int mainscreen =1, manualscreen=0, runscreen=0, mappingscreen=0, dummyscreen=0;
int pixel_x, pixel_y;    
bool down = false;

Adafruit_GFX_Button runsystem, stopsystem, startmapping, manual, stop, insert, add, removepoint, back_btn, start, cancel, dummy, startmap, maju, mundur, kanan, kiri, d_kanan_a, d_kiri_a, d_kanan_b, d_kiri_b, cw, ccw;
...
void loop()
{
  ...
  //dummy.press(down && dummy.contains(pixel_x, pixel_y));
  ...
  if (mainscreen == 1){
    ...
    if (runsystem.justPressed()){
      ...
      runscreen = 1;
      tft.fillScreen(BLACK);
      start.initButton(&tft,  400, 250, 70, 40, WHITE, CYAN, MAGENTA, "START", 2);
      cancel.initButton(&tft, 400, 300, 70, 40, WHITE, CYAN, MAGENTA, "CANCEL", 2);
    ...
    if (startmapping.justPressed()){
      ...
      mappingscreen= 1;
      tft.fillScreen(BLACK);
      startmap.initButton(&tft,  400, 250, 70, 40, WHITE, CYAN, MAGENTA, "START", 2);
      cancel.initButton(&tft, 400, 300, 70, 40, WHITE, CYAN, MAGENTA, "CANCEL", 2);
  ...
  if (manualscreen == 1){
    ...
    if (back_btn.justPressed()){
      ...
      Serial.println("back to homescreen");
  ...
  if (runscreen == 1){
    ...
    if (removepoint.justPressed()){
      removepoint.drawButton(true);
      Serial.println("remove");
      removepoint.press(false);
      delay(300);
      removepoint.drawButton(false);
    }
  }
